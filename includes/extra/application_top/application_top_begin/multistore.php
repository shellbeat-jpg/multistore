...
if(!isset($template)){
  ...
  if(defined("MULTISTORE") &&  MULTISTORE=='true'){   
    ...
    # D.L.
    if(!$template = preinitMultistore(true))
  ...
  ms_initConstants();  
  if(file_exists(('templates/'.CURRENT_TEMPLATE.'/lang/'.$_SESSION['language'].'.custom.php')))
	require (DIR_FS_CATALOG .'templates/'.CURRENT_TEMPLATE.'/lang/'.$_SESSION['language'].'.custom.php');	
  $debugSql=false;
}
