<?php

// This class was generated on Wed, 01 Aug 2018 16:35:39 PDT by version 0.1.0-dev+0ee05a-dirty of Braintree SDK Generator
// OrdersPatchTrackRequest.php
// @version 0.1.0-dev+0ee05a-dirty
// @type request
// @data 
// DO NOT EDIT

namespace PayPalCheckoutSdk\Orders;

use PayPalHttp\HttpRequest;

class OrdersPatchTrackRequest extends HttpRequest
{
    function __construct($orderId, $trackersId)
    {
        parent::__construct("/v2/checkout/orders/{order_id}/trackers/{trackers_id}", "PATCH");

        $this->path = str_replace("{order_id}", urlencode($orderId), $this->path);
        $this->path = str_replace("{trackers_id}", urlencode($trackersId), $this->path);
        $this->headers["Content-Type"] = "application/json";
    }



}
