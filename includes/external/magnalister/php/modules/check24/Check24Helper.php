...
class Check24Helper extends AttributesMatchingHelper {
    private static $instance;
    public static function gi() {
        if (self::$instance === null) {
            self::$instance = new Check24Helper();
        }
    ...
    protected function isProductPreparedForIndependent($prepare = false) {
        ...
        return MagnaDB::gi()->recordExists(TABLE_MAGNA_CHECK24_PROPERTIES, array(
            'MpId' => $this->mpId,
            $sKeyType => $prepare
        ));
    }
    public function getCategoryIndependentMatching() {
        //'magnalister_' . $this->marketplace . '_variantmatching'
        $tableName = $this->getVariationMatchingTableName();
    ...
    protected function getPreparedIndependentData($prepare = false, $customIdentifier = '', $dbField = 'NULL') {
        ...
        if ($_GET['where'] == 'varmatchView') {
            $availableCustomConfigs = json_decode(MagnaDB::gi()->fetchOne(eecho('
                    SELECT ShopVariation
                      FROM '.TABLE_MAGNA_CHECK24_VARIANTMATCHING.'
                     WHERE     MpId = '.$this->mpId.'
                     AND MpIdentifier = "category_independent_attributes"
                ', false), true), true);
    ...
    public function renderMatchingTable($url, $categoryOptions, $addCategoryPick = true, $displayCategory = true, $customIdentifierHtml = '') {
        ...
        <form method="post" id="matchingForm" action="<?php echo toURL($url, array(), true); ?>">
            <table id="variationMatcher" class="attributesTable">
                ...
                <tbody id="tbodyDynamicIndependentMatchingHeadline" style="">
                <tr class="even">
                    <th class="ottoGreyBackground"><h4><?php echo ML_CHECK24_CATEGORY_INDEPENDENT_ATTRIBUTES_REQUIRED ?></h4></th>
                    <td class="ottoGreyBackground" colspan="3"><h4><?php echo ML_CHECK24_CATEGORY_INDEPENDENT_ATTRIBUTES_REQUIRED_INFO ?></h4></td>
                </tr>
                </tbody>
                ...
                <tr class="even">
                    <th class="ottoGreyBackground"><h4><?php echo ML_CHECK24_CATEGORY_INDEPENDENT_ATTRIBUTES_OTIONAL ?></h4></th>
                    <td class="ottoGreyBackground" colspan="3"><h4><?php echo ML_CHECK24_CATEGORY_INDEPENDENT_ATTRIBUTES_OTIONAL_INFO ?></h4></td>
                </tr>
                </tbody>
